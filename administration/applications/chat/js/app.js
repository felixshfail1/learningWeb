 ...
 * @param {*} date 
 */
function dateParsor(date){
    cleanDate = ""
    year = date.substring(0,4)
    month = date.substring(5,7)
    day = date.substring(8,10)
    hhmm = date.substring(11,16)
    translatedMonth = ""
    ...
    cleanDate = day + " " + translatedMonth + " " + year + " à " + hhmm
    return cleanDate
}
...
function getMessages(){
    ...
    requeteAjax.onload = function(){
      ...
      const html = resultat.reverse().map(function(message){
        dateMessage = dateParsor(message.created_at)
        return `
       
          <div class="message" >
            <div class="messageDate" style="font-size : 8px;>
              <span class="date">${dateMessage}</span>
            </div>
